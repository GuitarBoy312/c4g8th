...
def generate_essay_question():
    ...
    '''
    prompt = f"""
    {name}과 {answer}을 이용하여CEFR A1 수준의 영어 문장을 1문장으로 작성해주세요.
    예시: {name}: {answer}
    그 다음, 지문에 관한 간단한 질문을 한국어로 만들어주세요. 
    질문을 만들 때, 화자를 포함해서 질문해 주세요. 예를 들어, 화자가 {name}이면 "{name}이..." 로"로 시작하는 질문을 생성해 주세요.
...
def generate_conversation_question():
    ...
    '''
    prompt = f"""
    {key_expression}과 같은 구문을 사용 하는 CEFR A1 수준의 간단한 영어 대화를 생성해주세요. 
    영어 대화를 생성할 때, 마지막 대화 내용은 알려주지 말고대화 내용에 관한 객관식 질문으로 만들어야 합니다. 
    그 후 대화 내용에 관한 객관식 질문을 한국어로 만들어주세요.  
    조건: 문제의 정답은 1개 입니다. 
    {name}와 {name}가 대화할 때 상대방의 이름을 부르면서 대화를 합니다. 
    ...
    if st.session_state.question_data:
        if st.session_state.question_type == "essay":
            ...
            with st.form(key='answer_form'):
                ...
                if submit_button:
                    if selected_option:
                        selected_letter = selected_option[0]  # 선택한 옵션의 첫 글자 (A, B, C, D)
